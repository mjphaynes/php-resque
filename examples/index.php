...
if (isset($_GET['action'])) switch ($_GET['action']) {
	...
	case 'push'            : $job = Resque::push('HelloWorld', array());break;
	case 'delayed'         : $job = Resque::later(mt_rand(0, 30), 'HelloWorld', array());break;
	case 'delayedat'       : $job = Resque::later(new \DateTime('+2 mins'), 'HelloWorld', array());break;
	case 'longrunning'     : $job = Resque::push('LongRunning', array());break;
	case 'failnoclass'     : $job = Resque::push('\Does\Not\Exist', array());break;
	case 'faillong'        : $job = Resque::push('FailLong', array());break;
	case 'failexception'   : $job = Resque::push('FailException', array());break;
	case 'failerror'       : $job = Resque::push('FailError', array());break;
	case 'closure'         : $job = Resque::push(function($job) {echo 'This is an inline job! #'.$job->getId().PHP_EOL;});break;
	case 'closure-delayed' : $job = Resque::later(mt_rand(0, 30), function($job) {echo 'This is a delayed inline job! #'.$job->getId().PHP_EOL;});break;
}
